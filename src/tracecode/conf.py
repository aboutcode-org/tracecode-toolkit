...
PUBLIC_OPTIONS = [
    ...
    Field('stats', False,
          '--stats',
          '''Print syscall statistics when parsing'''),
    ]
...
PURE_OPTIONS = [
    ...
    Field(None, None,
          '--pid=PID',
          '''With the debug command, only print the process with this PID.'''),
...
class Settings(object):
    ...
    def __repr__(self):
        ...
        return 'Settings(%s)' % pformat(self.__dict__,)
    ...
    def formatted(self):
        ...
        # empty list
        lists = set(f.name for f in PUBLIC_OPTIONS
                    if f.default in ([], tuple(),))
...
def BaseSettings(**kwargs):
    s = Settings(multiplexers=tuple(DEFAULT_MUXERS),
                 errors=tuple(DEFAULT_ERRORS),
                 **kwargs)
    return s


def DefaultSettings(**kwargs):
    return BaseSettings(ignored_reads=tuple(DEFAULT_IGNORED_READS),
...
def patterns_from_file(file_name):
    ...
    assert (os.path.exists(fn)
            and os.path.isfile(fn)), ('Pattern file %(file_name)s does not '
                                      'exist or is not a file.') % locals()
    if fn.endswith('.csv'):
...
def patterns(pat_list, pat_file_name):
    ...
    if isinstance(pat_list, basestring):
        pat_list = [pat_list]
...
def args_subset(args):
    ...
    settings_keys = set(s.name for s in PUBLIC_OPTIONS
                        if s.default != tuple())
    # remove leading dash from options that have one
    no_dash = dict([(k.lstrip('-'), v,) for k, v in args.items()
...
def settings(args=None):
    ...
    if args:
        ...
        stgs.ignored_reads += patterns(args.get('--ignored_reads'), args.get('--ignored_reads_from'))

        stgs.ignored_writes += patterns(args.get('--ignored_writes'), args.get('--ignored_writes_from'))

        stgs.ignored_execs += patterns(args.get('--ignored_execs'), args.get('--ignored_execs_from'))
